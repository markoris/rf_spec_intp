...
intp.load_data('/lustre/scratch4/turquoise/mristic/knsc1_active_learning/*spec*', '/lustre/scratch4/turquoise/mristic/h5_data/TP_wind2_spectra.h5', t_max=None, theta=float(args.theta), trim_dataset=True)
intp.create_test_set(size=5)
intp.append_input_parameter(intp.times, 1)
intp.preprocess()
intp.load('/lustre/scratch4/turquoise/mristic/rf_spec_intp_optim_trimdata_theta%sdeg.joblib' % args.theta)

param = intp.params_test[-1][:4]
param = np.concatenate((param, intp.times[np.argmin(np.abs(intp.times-10.4))].reshape(1))).reshape(1, -1) # 10-day time input
print(param)
