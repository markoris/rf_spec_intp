...
def calc_lc(tini,tmax,dt,mej,vej,beta,kappa_r,wav_trim=False):
    ...
    #print("Rphoto/D: ", Rphoto/D)

    #mAB = -2.5*np.log10(F) - 48.6

    # switch back to wavelengths, c is in cm/s, so convert wavs from nm to cm
    #lambdaobsarray = np.tile(lambdaobs*1e-7,(tprec,1)).T
    lambdaobsarray = c/nuobsarray
    F_lam = F*c/(lambdaobsarray**2)

    # distance modulus
    muD = 5.0*np.log10(D/(3.08e18))-5.

    #return tdays, Ltotm*1e40, mAB, Tobs
    return tdays, Ltotm*1e40, F_lam, Tobs
