def convert_dat_to_h5(path_to_sims_dir, path_to_h5_out, cutoff=42):
	...
	if cutoff > 38.055:
		print("Warning: Not all spectra go out to %f days! Setting to common max time of 38.055 days." % cutoff)
		cutoff = 38.055
	...
	for idx in range(files.shape[0]):
		...
		spec_data = spec_data[:t_idx*1024, 2:56] # change this hard cutoff of 60 to something user-specific, find greatest common index
		try:
			all_data = np.concatenate((all_data, spec_data), axis=0)
		except NameError:
			all_data = spec_data
		print('Concatenated %d out of %d spectra' % (all_data.shape[0]/(t_idx*1024), files.shape[0]))
